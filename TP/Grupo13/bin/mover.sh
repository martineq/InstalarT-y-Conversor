...
main() {
    ...
    #Si trajo más de dos parametros entonces trajo el tipo de mensaje
    if [ $# \> 2 ] 
    then
        COMANDO=$3
    else
    ...
    if [ $? == 0 ]; then
        #Si tengo más de dos parametros entonces tengo el comando, por lo tanto genero log para el comando.
        if [ $# \> 2 ]; then
